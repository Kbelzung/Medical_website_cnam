<?php 
    ...
    if(!empty($_GET['id']) && !empty($_GET['firstname']) && !empty($_GET['lastname']) && !empty($_GET['doctor_type']) && !empty($_GET['email']) && !empty($_GET['phone'])) {
                // Patch XSS
                ...
                if($row > 0){
                    if(strlen($email) <= 100){ // check email size
                        if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // validate mail format
                                $insert = $bdd->prepare('UPDATE doctor SET email = :email, first_name = :first_name, last_name = :last_name, title = :title, phone = :phone WHERE id = :id');
